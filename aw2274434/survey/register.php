...
<!-------------------------------------Register------------------------------------->
				...
				if ($_SERVER['REQUEST_METHOD'] == 'POST') {
					...
					if 	(empty($trimmed['username'])){
						...
						if ($result = @mysqli_query($dbc, 'SELECT * FROM aw2274434_survey_users WHERE username = "'.$trimmed['username'].'"'))	{
					...
					if ($result = @mysqli_query($dbc, 'SELECT * FROM aw2274434_survey_users WHERE email = "'.$trimmed['email'].'"'))	{
							if (mysqli_num_rows($result) > 0){
					...
					if (empty($errors)) {
						...
						$q = "INSERT INTO aw2274434_survey_users (username, first_name, last_name, email, pass, reg_date) VALUES ('$un', '$fn', '$ln', '$e', SHA1('$p'), NOW() )";		
				...
				<form action="register.php" method="post">
                	<p>Username: <input type="text" name="username" size="15" maxlength="20" value="<?php if (isset($trimmed['username'])) echo $trimmed['username']; ?>" /></p></br>
					<p>First Name: <input type="text" name="first_name" size="15" maxlength="20" value="<?php if (isset($trimmed['first_name'])) echo $trimmed['first_name']; ?>" /></p></br>
					...
					<p>Email Address: <input type="text" name="email" size="20" maxlength="60" value="<?php if (isset($trimmed['email'])) echo $trimmed['email']; ?>"  /> </p></br>
					<p>Password: <input type="password" name="pass1" size="10" maxlength="40" value="<?php if (isset($trimmed['pass1'])) echo $trimmed['pass1']; ?>"  /></p></br>
					<p>Confirm Password: <input type="password" name="pass2" size="10" maxlength="20" value="<?php if (isset($trimmed['pass2'])) echo $trimmed['pass2']; ?>"  /></p></br>
