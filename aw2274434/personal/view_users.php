...
<!-------------------------------------View Users------------------------------------->
			...
			if (isset($_GET['p']) && is_numeric($_GET['p'])) {
				...
				$q = "SELECT COUNT(user_id) FROM aw2274434_personal_users";
			...
			$q = "SELECT username, last_name, first_name, DATE_FORMAT(reg_date, '%M %d, %Y') AS dr, user_id FROM aw2274434_personal_users ORDER BY $order_by LIMIT $start, $display";		
			$r = @mysqli_query ($dbc, $q);
