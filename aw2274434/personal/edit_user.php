...
<!-------------------------------------Edit User------------------------------------->
		...
		<?php 
			...
			if (empty($errors)) { 
				$q = "SELECT user_id FROM aw2274434_personal_users WHERE email='$e' AND user_id != $id";
				$r = @mysqli_query($dbc, $q);
				if (mysqli_num_rows($r) == 0) {
					$q = "UPDATE aw2274434_personal_users SET first_name='$fn', last_name='$ln', email='$e' WHERE user_id=$id LIMIT 1";
					$r = @mysqli_query ($dbc, $q);
			...
			$q = "SELECT first_name, last_name, email FROM aw2274434_personal_users WHERE user_id=$id";		
			$r = @mysqli_query ($dbc, $q);
